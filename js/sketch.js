...
function loadLevel(n) {
  ...
  if (!grid) {
    grid = new Grid(0, 0, 0, 0, levels[n].grid);
  } else {
    grid.setGrid(levels[n].grid);
    grid.reset();
  }
...
function computeDimensions() {
  ...
  grid.setDimensions(gridWidth, gridHeight);

  hand.setPosition(handX, handY);
  hand.setDimensions(handWidth, handHeight);
...
function draw() {
  ...
  textSize(32);
  fill(255);
  text("Level " + levels[currentLevel].id, grid.x + 20, grid.y + 20);
